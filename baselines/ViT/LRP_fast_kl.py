...
# ====================== Model Definitions with Local Loading ======================
''''
def vit_base_patch16_224(pretrained=False, keep_rate=None, **kwargs):
    if keep_rate is None:
        keep_rate = tuple([1.0] * 12)
    
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, 
        mlp_ratio=4, qkv_bias=True, keep_rate=keep_rate, **kwargs)
    
    model.default_cfg = default_cfgs['vit_base_patch16_224']
    
    if pretrained:
        checkpoint_path = "/content/drive/MyDrive/STAR/pretrained_models/jx_vit_base_p16_224-80ecf9dd.pth"
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        model.load_state_dict(checkpoint, strict=False)
        print("✅ ViT-Base weights loaded from Google Drive")
    return model


def vit_large_patch16_224(pretrained=False, keep_rate=None, **kwargs):
    if keep_rate is None:
        keep_rate = tuple([1.0] * 24)
    ...
    if pretrained:
        checkpoint_path = "/content/drive/MyDrive/STAR/pretrained_models/jx_vit_large_p16_224-4ee7a4dc.pth"
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        model.load_state_dict(checkpoint, strict=False)
        print("✅ ViT-Large weights loaded from Google Drive")
    return model


def deit_base_patch16_224(pretrained=False, keep_rate=None, **kwargs):
    if keep_rate is None:
        keep_rate = tuple([1.0] * 12)
    ...
    if pretrained:
        checkpoint_path = "/content/drive/MyDrive/STAR/pretrained_models/DeiT-Base.pth"
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        model.load_state_dict(checkpoint["model"])
        print("✅ DeiT-Base weights loaded from Google Drive")
    return model


def deit_small_patch16_224(pretrained=False, keep_rate=None, **kwargs):
    if keep_rate is None:
        keep_rate = tuple([1.0] * 12)
    ...
    if pretrained:
        checkpoint_path = "/content/drive/MyDrive/STAR/pretrained_models/DeiT-Small.pth"
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        model.load_state_dict(checkpoint["model"])
        print("✅ DeiT-Small weights loaded from Google Drive")
    return model
'''
def deit_tiny_patch16_224(pretrained=False, keep_rate=None, **kwargs):
    if keep_rate is None:
        keep_rate = tuple([1.0] * 12)
    ...
    if pretrained:
        checkpoint_path = "/content/drive/MyDrive/STAR/pretrained_models/DeiT-Tiny.pth"
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        model.load_state_dict(checkpoint["model"])
        print("✅ DeiT-Tiny weights loaded from Google Drive")
    return model

# ====================== Create the Model ======================

# Your custom keep rate
keep_rate = [1.00, 0.95, 0.92, 0.88, 0.82, 0.78, 0.72, 0.65, 0.58, 0.52, 0.48, 0.45]


# model = deit_small_patch16_224(pretrained=True, keep_rate=keep_rate).cuda()
# model = deit_base_patch16_224(pretrained=True, keep_rate=keep_rate).cuda()
model = deit_tiny_patch16_224(pretrained=True, keep_rate=keep_rate).cuda()
